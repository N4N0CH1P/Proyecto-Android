...
class RegisterWindow : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var arrayIds: Array<Int> = arrayOf(R.id.editNombre,R.id.editApellido,R.id.editEmail,R.id.editFechaNacimiento)
            ...
            if(passwd1.text.toString()==passwd2.text.toString()){
                ...
                if(NetworkConnection.isNetworkConnected(this)){
                    ...
                    for(i in 0 until arrayLlavesPost.size){
                        ...
                        datosPost += URLEncoder.encode(arrayLlavesPost[i], "UTF-8") + "=" + URLEncoder.encode(arrayInputs[i].text.toString(), "UTF-8")+"&"
                    }
                    //agregar lo faltante
                    datosPost+=URLEncoder.encode("password", "UTF-8") + "=" + URLEncoder.encode(passwd1.text.toString(), "UTF-8")+"&"
                    datosPost+=URLEncoder.encode("rango", "UTF-8") + "=" + URLEncoder.encode(rango, "UTF-8")+"&"
                    datosPost+=URLEncoder.encode("sexo", "UTF-8") + "=" + URLEncoder.encode(sexo.toString(), "UTF-8")
                    //llamar la funcion para registrar usuario
                    registerUser(datosPost)
                }
                else{
                    Toast.makeText(this, "No se econtro la conexion a Internet", Toast.LENGTH_LONG).show()
                }
            }
            else{
                Toast.makeText(this, "Las contrasenias no considen", Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun verifyRegister(respuestaServicio:String){
        ...
        if(objetoJsonRespuesta.has("success")){
            ...
            returnIntent.putExtra(MainActivity.EMAIL, email)
            returnIntent.putExtra(MainActivity.PASSWORD,password)
